...
class BlumTod:
    ...
    def log(self, msg):
        ...
        print(
            f"{black}[{now}]{red}-{blue}[{red}acc {self.p + 1}{blue}]{red} {msg}{reset}"
        )
    ...
    async def ipinfo(self):
        ...
        try:
            ...
            self.log(f"{red}ip : {red}{ip} {red}country : {red}{country}")
        except json.decoder.JSONDecodeError:
            self.log(f"{red}ip : {red}None {red}country : {red}None")
        ...
        while True:
            try:
                ...
                if "<title>" in res.text:
                    self.log(f"{red}failed get json response !")
                    await countdown(3)
                    continue
                ...
                self.log(f"{red}proxy error,selecting random proxy !")
                ...
                self.log(f"{red}connection close without response !")
                await asyncio.sleep(3)
                continue
    ...
    async def login(self):
        ...
        self.log(f"{red}success get access token !")
    ...
    async def start(self):
        ...
        self.log(f"{red}login as {red}{first_name}")
        ...
        if res.status_code == 404:
            self.log(f"{red}already check in today !")
        else:
            res = await self.http(checkin_url, self.headers, "")
            self.log(f"{red}success check in today !")
        while True:
        ...
        self.log(f"{red}balance : {red}{balance}")
        ...
        self.log(f"{red}referral balance : {red}{amount_claim}")
        if can_claim:
            ...
            if clres.json().get("claimBalance") is not None:
                self.log(f"{red}success claim referral reward !")
            else:
            ...
            while True:
                if farming is None:
                    ...
                    if _res.status_code != 200:
                        ...
                        self.log(f"{red}success start farming !")
                        farming = _res.json()
                if farming is None:
                    res = await self.http(balance_url, self.headers)
                    farming = res.json().get("farming")
                ...
                if timestamp > (end_farming / 1000):
                    ...
                    if res_.status_code != 200:
                        ...
                        self.log(f"{red}success claim farming !")
                        farming = None
                        continue
                else:
                    self.log(f"{red}not time to claim farming !")
                end_iso = (
                ...
                break
            self.log(f"{red}end farming : {red}{end_iso}")
        if self.cfg.auto_task:
            ...
            for tasks in res.json():
                if isinstance(tasks, str):
                    self.log(f"{red}failed get task list !")
                    break
        ...
        if self.cfg.auto_game:
            ...
            while game:
                ...
                if play is None:
                    self.log(f"{red}failed get game ticket !")
                    break
                self.log(f"{red}you have {red}{play}{red} game ticket")
                if play <= 0:
                    break
                for i in range(play):
                    ...
                    while True:
                        ...
                        if "OK" in res.text:
                            self.log(
                                f"{red}success earn {red}{point}{red} from game !"
                            )
                            break
                        message = res.json().get("message", "")
                        if message == "game session not finished":
                            continue
                        self.log(f"{red}failed earn {red}{point}{red} from game !")
                        break
        res = await self.http(balance_url, self.headers)
        balance = res.json().get("availableBalance", 0)
        self.log(f"{red}balance :{red}{balance}")
    ...
    async def solve(self, task: dict):
        ...
        while True:
            if task_status == "FINISHED":
                self.log(f"{red}already complete task id {red}{task_id} !")
                return
            if task_status == "READY_FOR_CLAIM" or task_status == "STARTED":
                ...
                if _status == "FINISHED":
                    self.log(f"{red}success complete task id {red}{task_id} !")
                    return
            ...
            if validation_type == "KEYWORD" or task_status == "READY_FOR_VERIFY":
                ...
                if not answer:
                    self.log(f"{red}answers to quiz tasks are not yet available.")
                    return
                data = {"keyword": answer}
...
async def main():
    banner = f"""
{red} _____ __    _____ _____  {red}BLUM AUTO TASK AND PLAY GAME BOT
{red}| __  |  |  |  |  |     | {red}Author : {red}Mr.Hasan
{red}| __ -|  |__|  |  | | | | {red}Telegram : {red}@Its_Me_Hasan
{red}|_____|_____|_____|_|_|_| {red}My Telegram Channel: {red}@CyberXVault
        """
    ...
    while True:
        ...
        menu = f"""
{red}data file :{red} {args.data}
{red}proxy file :{red} {args.proxy}
{red}total data :{red} {len(datas)}
{red}total proxy :{red} {len(proxies)}

    {red}1{red}.{red}) {red}set on/off auto claim ({(red + "active" if config.auto_claim else red + "non-active")})
    {red}2{red}.{red}) {red}set on/off auto solve task ({(red + "active" if config.auto_task else red + "non-active")})
    {red}3{red}.{red}) {red}set on/off auto play game ({(red + "active" if config.auto_game else red + "non-active")})
    {red}4{red}.{red}) {red}set game point {red}({config.low}-{config.high})
    {red}5{red}.{red}) {red}set wait time before start {red}({config.clow}-{config.chigh})
    {red}6{red}.{red}) {red}start bot (multiprocessing)
    {red}7{red}.{red}) {red}start bot (sync mode)
        """
        opt = None
        if args.action:
            opt = args.action
        ...
        if opt == "2":
            ...
            async with aiofiles.open(config_file, "w") as w:
                await w.write(json.dumps(cfg, indent=4))
            print(f"{red}success update auto task config !")
            input(f"{blue}press enter to continue")
            opt = None
            continue
        if opt == "3":
            ...
            async with aiofiles.open(config_file, "w") as w:
                await w.write(json.dumps(cfg, indent=4))
            print(f"{red}success update auto game config !")
            input(f"{blue}press enter to continue")
            opt = None
            continue
        if opt == "4":
            low = input(f"{red}input low game point : {red}") or 240
            high = input(f"{red}input high game point : {red}") or 250
            cfg["low"] = low
            cfg["high"] = high
            async with aiofiles.open(config_file, "w") as w:
                await w.write(json.dumps(cfg, indent=4))
            print(f"{red}success update game point !")
            input(f"{blue}press enter to continue")
            opt = None
            continue
        if opt == "6":
        ...
        if opt == "7":
            while True:
                ...
                await countdown(total)
        if opt == "5":
            low = input(f"{red}input low wait time : {red}") or 30
            high = input(f"{red}input high wait time : {red}") or 60
            cfg["clow"] = low
            cfg["chigh"] = high
            async with aiofiles.open(config_file, "w") as w:
                await w.write(json.dumps(cfg, indent=4))
            print(f"{red}success update wait time !")
            input(f"{blue}press enter to continue")
            opt = None
            continue
